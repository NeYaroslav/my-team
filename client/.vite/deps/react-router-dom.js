...
function createRouter(init) {
  ...
  function initialize() {
    ...
    let lazyPromises = lazyMatches.map((m) =>
      loadLazyRouteModule(m.route, detectErrorBoundary2, manifest)
    )
    Promise.all(lazyPromises).then(() => {
      let initialized2 =
        !state.matches.some((m) => m.route.loader) || init.hydrationData != null
      if (initialized2) {
        updateState({
          initialized: true,
        })
  ...
  async function handleFetcherLoader(
    ...
    if (isDeferredResult(result)) {
      result =
        (await resolveDeferredData(result, fetchRequest.signal, true)) || result
    }
    if (fetchControllers.get(key) === abortController) {
      fetchControllers.delete(key)
    }
    ...
    if (match.route.lazy) {
      if (handler) {
        let values = await Promise.all([
          runHandler(handler),
          loadLazyRouteModule(match.route, detectErrorBoundary2, manifest),
        ])
        result = values[0]
      } else {
